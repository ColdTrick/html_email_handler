 ...
 * @param bool   $return The current hook values containing:
 *                       to => who to send the email to
 *                       from => who is the sender
 *                       subject => subject of the message
 *                       body => message
 *                       params => optional params
 * @param array $params  Original hook parameters
 *
 * @return bool
 */
function html_email_handler_email_hook($hook, $type, $return, $params) {
	// generate HTML mail body
	$return['html_message'] = html_email_handler_make_html_body($return);
	$return['plaintext_message'] = $return['body'];
	
	return html_email_handler_send_email($return);
}
